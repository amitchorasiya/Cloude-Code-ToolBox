...
object BundledMcpRecipeIntellij {
    ...
    fun run(project: Project) {
        ...
        val stream = javaClass.classLoader.getResourceAsStream("mcp-recipes/bundled-recipes.json")
            ?: run {
                Messages.showErrorDialog(project, "Bundled MCP recipes missing from plugin.", "Cloude Code ToolBox")
                return
            }
        ...
        val data = try {
            ...
            Messages.showErrorDialog(project, "Invalid bundled recipes JSON.", "Cloude Code ToolBox")
            return
        }
        val recipes = data.getAsJsonArray("recipes") ?: run {
            Messages.showErrorDialog(project, "No recipes defined.", "Cloude Code ToolBox")
            return
        }
        if (recipes.size() == 0) {
            Messages.showErrorDialog(project, "No recipes defined.", "Cloude Code ToolBox")
        ...
        if (serverKey.isEmpty()) {
            Messages.showErrorDialog(project, "Invalid recipe serverKey.", "Cloude Code ToolBox")
            return
        }
        val cfgEl = recipe.get("config") ?: run {
            Messages.showErrorDialog(project, "Invalid recipe config.", "Cloude Code ToolBox")
            return
        }
        ...
        if (mergedConfig == null || !mergedConfig.isJsonObject) {
            Messages.showErrorDialog(project, "Could not build server config.", "Cloude Code ToolBox")
            return
        }
        ...
        if (servers.has(serverKey)) {
            val ok = Messages.showYesNoDialog(
                project,
                "Server \"$serverKey\" already exists in mcp.json. Overwrite?",
                "Overwrite server",
                Messages.getWarningIcon(),
            )
            if (ok != Messages.YES) return
        }
        servers.add(serverKey, mergedConfig)
        ...
        Messages.showInfoMessage(project, "Updated .vscode/mcp.json (server \"$serverKey\").", "Cloude Code ToolBox")
