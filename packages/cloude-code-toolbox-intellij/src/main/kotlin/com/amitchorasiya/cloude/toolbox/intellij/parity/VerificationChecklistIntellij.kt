...
object VerificationChecklistIntellij {
    ...
    fun run(project: Project) {
        ...
        if (n == 0) {
            Messages.showInfoMessage(project, "Verification: no items checked.", "Cloude Code ToolBox")
        } else {
            Messages.showInfoMessage(project, "Verification: $n item(s) acknowledged.", "Cloude Code ToolBox")
        }
    }
}
...
private class VerificationDialog(project: Project) : DialogWrapper(project) {
    ...
    init {
        title = "Verification checklist (Cloude Code ToolBox)"
